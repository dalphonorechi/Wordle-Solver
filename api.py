...
def get_word(correct="?????", pattern_not="", pattern=""):
...
def get_words(track):
    ...
    """
    print(track.correct)
    pattern = get_patterns(track)[0]

    pattern_absent = get_patterns(track)[1]
    ...
    temp_c = words.copy()
    for k in words:
        b = check_word(k, track.guessed, create_check(track.guessed, track.evaluations))
        if b:
            temp_c.remove(k)
